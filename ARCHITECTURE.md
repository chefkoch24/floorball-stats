...
`floorball-stats` is a Python-based analytics and publishing pipeline for German floorball league data.
...
```text
Saisonmanager API (https://saisonmanager.de/api/v2)
    -> src/scrape.py
...
- `content/`
  - Pelican source content by season and type (`teams`, `liga`, `games`).
- `generated/`
...
### 4.6 Markdown Generation: `src/generate_markdown.py`
...
### 4.7 Site Build: Pelican (`pelicanconf.py`, `themes/my-theme`)
...
1. The repository is currently in an active Git rebase state, which is operational risk, not runtime architecture.
