...
def test_playoff_split_respects_configured_playoff_cut_from_regular_season(tmp_path: Path):
    data_dir = tmp_path / "data"
    data_dir.mkdir(parents=True, exist_ok=True)
    ...
    regular_events = pd.DataFrame(
        [
            _event(1, "2026-02-01", "goal", "Team 1", "Team 1", "Team 7", 3, 0, 3, "3-19:00", "3-0"),
    ...
    playoffs_events.to_csv(playoffs_csv, index=False)
    ...
    playoff_teams = {team["team"] for team in result["playoff_stats"]}
    playdown_teams = {team["team"] for team in result["playdown_stats"]}
